...
from tia_tags_exporter.hmi_exporter import HmiExportResult, HmiExporter, HmiTargetInfo
...
class HmiExportWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, store, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        self.cmbFormat.addItem("Siemens AML folder (one folder per target)", "aml-folder")
        ...
        self.targetList.setSelectionMode(QtWidgets.QAbstractItemView.SelectionMode.ExtendedSelection)
    ...
    def set_session(self, session) -> None:
        ...
        project_name = getattr(session, "project_name", None) or getattr(session, "project", None)
        if project_name:
    ...
    def _prompt_output_folder(self) -> Optional[Path]:
        directory = QtWidgets.QFileDialog.getExistingDirectory(self, "Choose export folder")
        return Path(directory) if directory else None

    def _prompt_zip_path(self) -> Optional[Path]:
        file_path, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save AML Zip", "HMI_Exports.zip", "Zip (*.zip)")
        if not file_path:
            return None
    ...
    def _on_run_clicked(self) -> None:
        ...
        if exporter is None:
            QtWidgets.QMessageBox.warning(self, "HMI Export", "Attach to a project with HMI targets first.")
            return
    ...
    def _run_folder_export(self, identifiers: Optional[Iterable[str]], exporter: HmiExporter) -> None:
        ...
        try:
            results = exporter.export_targets(out_dir, target_filter=identifiers, progress=self._append_log)
        except Exception as exc:  # pragma: no cover - requires Openness runtime
            ...
            return
        QtWidgets.QApplication.restoreOverrideCursor()
        manifest_payload = _build_manifest(results, destination=out_dir, relative_root=out_dir)
        self._handle_results(results, out_dir, manifest_payload)

    def _run_zip_export(self, identifiers: Optional[Iterable[str]], exporter: HmiExporter) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="tia-hmi-export-") as tmp_dir:
                ...
                results = exporter.export_targets(tmp_root, target_filter=identifiers, progress=self._append_log)
                if not results:
                    QtWidgets.QApplication.restoreOverrideCursor()
                    QtWidgets.QMessageBox.information(self, "HMI Export", "No HMI targets were exported.")
                    return
                manifest_payload = _build_manifest(results, destination=zip_path, relative_root=tmp_root)
    ...
    def _handle_results(
        ...
        message = "Exported {} HMI target(s).\nOutputs stored at:\n{}".format(len(results), destination)
        ...
        manifest_targets.append(
            {
                ...
                "extras": {key: format_path(value) for key, value in sorted(result.extras.items())},
            }
        )

...
if __name__ is None or __name__ == "":
...
from tia_tags_exporter.devices_networks_exporter import DevicesNetworksExporter
from tia_tags_exporter.excel_writer import (
    write_devices_networks_csv,
    write_devices_networks_google_sheets,
    write_devices_networks_xlsx,
    write_tags_csv,
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, store: ProfileStore):
        ...
        self.cmbDevicesNetworksFormat.addItem("CSV (.csv)", "csv")
        self.cmbDevicesNetworksFormat.addItem("Excel (.xlsx)", "xlsx")
        self.cmbDevicesNetworksFormat.addItem("Google Sheets", "gsheet")
        self.cmbDevicesNetworksFormat.setCurrentIndex(0)

        devices_networks_row = QtWidgets.QHBoxLayout()
        devices_networks_row.addWidget(self.btnExportDevicesNetworks)
        devices_networks_row.addWidget(QtWidgets.QLabel("Format:"))
        devices_networks_row.addWidget(self.cmbDevicesNetworksFormat)
        devices_networks_row.addStretch(1)
        ...
        layout.addLayout(devices_networks_row)
    ...
    def _apply_theme(self, dark: bool) -> None:
        ...
        if dark:
            ...
            palette.setColor(
                QtGui.QPalette.ColorRole.ToolTipBase, QtGui.QColor(53, 53, 53)
            )
            palette.setColor(
                QtGui.QPalette.ColorRole.ToolTipText, QtGui.QColorConstants.White
            )
    ...
    def on_attach(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(
                self, "Attach", f"Failed to attach:\n{e}\n\nDiagnostics:\n{diagnostics}"
            )
    ...
    def on_export_devices_networks(self):
        if not self._session:
            QtWidgets.QMessageBox.warning(
                self, "Export Devices & Networks", "Attach to TIA first."
            )
            return
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(
                ...
                f"Failed to gather devices and networks:\n{err}",
            )
            return
        ...
        fmt = self.cmbDevicesNetworksFormat.currentData()

        if fmt == "xlsx":
            out, _ = QtWidgets.QFileDialog.getSaveFileName(
                self,
                "Save Devices & Networks Excel",
                "Devices_Networks.xlsx",
                "Excel (*.xlsx)",
            )
            if not out:
                return
            ...
            if out_path.suffix.lower() != ".xlsx":
                out_path = out_path.with_suffix(".xlsx")
            write_devices_networks_xlsx(rows, out_path)
            message = f"Exported {len(rows)} rows to:\n{out_path}"
            self.log.append(
                f"Exported {len(rows)} devices and networks rows -> {out_path}"
            )
            QtWidgets.QMessageBox.information(self, "Done", message)
            return
        ...
        if fmt == "csv":
            ...
            if out_path.suffix.lower() != ".csv":
                out_path = out_path.with_suffix(".csv")
            write_devices_networks_csv(rows, out_path)
            message = f"Exported {len(rows)} rows to:\n{out_path}"
            self.log.append(
                f"Exported {len(rows)} devices and networks rows -> {out_path}"
            )
        ...
        title, ok = QtWidgets.QInputDialog.getText(
            self, "Google Sheets", "Spreadsheet title:", text="Devices & Networks"
        )
        if not ok or not title.strip():
            return
        share_email, ok = QtWidgets.QInputDialog.getText(
        ...
        try:
            info = write_devices_networks_google_sheets(
                rows, creds_path, title.strip(), share_with=share_email or None
            )
        except RuntimeError as err:
            QtWidgets.QMessageBox.critical(self, "Google Sheets", str(err))
    ...
    def on_export_blocks(self):
        if not self._session:
            QtWidgets.QMessageBox.warning(
                self, "Export Program Blocks", "Attach to TIA first."
            )
            return
        ...
        if fmt == "xlsx":
            out, _ = QtWidgets.QFileDialog.getSaveFileName(
                self,
                "Save Program Blocks Excel",
                "PLC_Program_Blocks.xlsx",
                "Excel (*.xlsx)",
            )
            if not out:
                return
            ...
            if extras:
                message += "\nSaved {} source files to {}".format(
                    len(extras), extras[0].parent
                )
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(
                self,
                "Google Sheets",
                "Failed to export program blocks:\n{}".format(err),
            )
            return
